import { pgTable, index, pgPolicy, uuid, text, timestamp, foreignKey, check, integer, boolean, unique, uniqueIndex, numeric } from "drizzle-orm/pg-core"
...
export const gameSessions = pgTable("game_sessions", {
	...
	allowHostToPlay: boolean("allow_host_to_play").default(true).notNull(),
	totalRounds: integer("total_rounds").default(10).notNull(),
	enableTextInputMode: boolean("enable_text_input_mode").default(true).notNull(),
...
export const players = pgTable("players", {
	...
	spotifyUserId: text("spotify_user_id"), // Spotify user ID for authenticated hosts
}, (table) => [
	index("idx_players_is_host").using("btree", table.sessionId.asc().nullsLast().op("bool_ops"), table.isHost.asc().nullsLast().op("uuid_ops")).where(sql`(is_host = true)`),
...
export const gameRounds = pgTable("game_rounds", {
	...
	uniqueIndex("unique_buzzer_per_round").using("btree", table.sessionId.asc().nullsLast().op("int4_ops"), table.roundNumber.asc().nullsLast().op("int4_ops")).where(sql`(buzzer_player_id IS NOT NULL)`),
	foreignKey({
...
export const packTracks = pgTable("pack_tracks", {
	...
	index("idx_pack_tracks_position").using("btree", table.packId.asc().nullsLast().op("uuid_ops"), table.position.asc().nullsLast().op("int4_ops")),
